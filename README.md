...
当前状态：第 0 阶段第 1 课进行中，第一阶段三课已完成、新增第 4 课「优化器」（三件套已搭好待填），第二阶段 6 课全部完成。**第三阶段第 1 课 LoRA self_write 已全部完成（6/6 TODO）**；**第 2 课量化开学**——已吃透浮点格式对比（FP16 换精度/BF16 换范围）与对称量化（scale=max|x|/qmax、round 就近取整+to(int8) 省存储、对称范围 [-127,127] 保 0 精确映射）。下一步：填量化 self_write TODO-1/2。
