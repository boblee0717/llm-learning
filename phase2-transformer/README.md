...
| 入口 | 跳转 | 用途 |
|------|------|------|
| 当前进度 | [自写练习与重置](#progress) | 确认已经完成到哪一步、下一步跑什么 |
| 统一节奏 | [每课统一节奏](#rhythm) | 每课固定 6 步学习流程 |
| 第 1 课 | [词嵌入与位置编码](#lesson-1) | token embedding、position encoding |
| 第 2 课 | [自注意力机制](#lesson-2) | Q/K/V、scaled dot-product、causal mask |
...
- 🚧 第 5 课：主课 `05_gpt_from_scratch.py` 已跑通，自写练习 `05_gpt_from_scratch_self_write.py`（12 个 TODO，首次用 PyTorch，已融合主课练习区全部 6 道题）待填（重置脚本：`reset_exercises_05.py`）
...
2. **② 读论文**（15-20 min）：
   ...
   - `Sparse Transformer` → [Generating Long Sequences with Sparse Transformers](../papers/efficient-transformers/Generating_Long_Sequences_with_Sparse_Transformers_2019.pdf)（可选，理解 GPT-3 里的 locally banded sparse attention）
   - `BERT` → `Model Architecture`（对照 encoder 结构中的多头注意力）；逐句精读见 [`papers/notes/bert_reading_model_architecture.md`](../papers/notes/bert_reading_model_architecture.md)
...
5. **⑤ 动手写**：完成 `05_gpt_from_scratch_self_write.py`（12 个 TODO，每填一个就跑一次依靠 `require_*` 校验即时纠错：因果掩码 → QKV 分头 → 注意力核心计算 → FFN → Pre-Norm Block → weight tying → GPT.forward → get_batch → Top-K 过滤 → generate →（进阶可跳过）Top-P →（进阶可跳过）tiktoken BPE 分词；全部通过后脚本会用你写的模型真的训练 300 步并生成文本，支持换成 tiny-shakespeare 真实数据或自定义文本）；做完再调整 temperature 和 top-k 参数，感受不同采样策略的效果差异；主课练习区的 6 道题已全部融合进自写练习
