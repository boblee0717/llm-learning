...
- Multi-Head Attention 是把 Q/K/V 投影到多个较小子空间并行做 attention，再把多个 head 的输出拼接（concat）起来，最后用 `W^O` 投回 `d_model`。
...
### 句 5：Transformer 沿用此框架，但内部换成 self-attention + FFN
...
> **术语：FFN（Feed-Forward Network，前馈网络）** 在论文 §3.3 给出了具体定义：一个**两层 MLP + ReLU**，`FFN(x) = max(0, x W_1 + b_1) W_2 + b_2`，维度变化是 `d_model → d_ff → d_model`，base model 里就是 `512 → 2048 → 512`（先升维 4 倍，再降回来）。**MLP（Multi-Layer Perceptron，多层感知机）** 可以先简单理解成「多层全连接网络」：`Linear → 激活函数 → Linear`。在 Transformer 语境里，FFN 就是这个 MLP 模块；它在不同文献里也被叫作 `MLP` / `position-wise FFN` / `pointwise FFN`，本质是同一个东西。
>
> 这里的「两层」指两个 Linear / 全连接层：第一层是 `x W_1 + b_1`，第二层是 `(...) W_2 + b_2`。中间的 `max(0, ·)` 不是第三层 Linear，而是 **ReLU 激活函数**：小于 0 的值变成 0，大于 0 的值保留。它负责加入非线性；如果没有这一步，两层 Linear 可以合并成一层 Linear，表达能力会弱很多。

`point-wise` 的意思是：FFN 对每个 token 位置独立施加**同一套** MLP 参数，**不在位置之间混合信息**。位置之间的混合完全交给 self-attention 完成。这是 Transformer 「分工清晰」的关键：

- self-attention 负责「token 之间」的信息流动（横向混合）
- FFN 负责「单个 token 内部」特征的非线性加工（纵向加工）
...
为什么要堆 6 层？直觉上，每一层负责把表示「再精炼一次」：浅层可能学到局部特征（搭配、词性），深层可能学到更抽象的关系（指代、语法结构）。

> **`6` 不是理论推导，是实验调出来的折中值。** 论文 §6.2 Table 3 做过层数消融：`N = 2, 4` 效果掉得明显（容量不够），`N = 8` 又没有显著提升（收益递减），所以 base model 取了 `6`。它是 2017 年那个数据规模（WMT 4.5M 句对）+ 算力档位下的最优解，**不是 Transformer 架构的固有数字**——后续模型早就改了：BERT-base 12 层、BERT-large 24 层、GPT-3 96 层、LLaMA-2 70B 80 层。随算力/数据规模增长，最优深度一路在涨。
>
> 另外注意：论文里的 **big model** 也仍然是 `N = 6`，作者优先把模型「**加宽**」（`d_model` 512→1024，`d_ff` 2048→4096，`h` 8→16）而不是加深。这也说明在那个尺度下，加宽比加深更划算。
...
#### 句 8：两个 sub-layer 分别是 self-attention 和 position-wise FFN
...
- **self-attention**：Q、K、V 都来自**同一个序列**。在 encoder 里就是上一层 encoder 的输出，每个 token 都能看见同一句话里的所有 token——encoder 一次性处理完整输入，不存在「时间方向」，所以**不需要 causal mask**。但实战中通常仍要加 **padding mask**（屏蔽 `<pad>` 位置），这是另一回事。
...
#### 句 9：每个 sub-layer 外面包残差 + LayerNorm
...
这里的「连接」指的是输入 `x` 这条**跳过 sub-layer、直接走到 Add 位置**的路径；`Add` 指最后真正执行的相加动作：

```text
output = x + Sublayer(x)
```

所以严格说，**残差连接**是 `x` 这条直连路径，**残差相加**是最后的 `x + Sublayer(x)`。日常说「残差连接」时，经常把这两件事合在一起表达：保留原输入 `x`，再和 sub-layer 的输出相加。
...
#### 句 10：统一公式 LayerNorm(x + Sublayer(x))（Post-Norm）
...
这里的 `N = 6` 和 encoder 一样，主要是原论文的**对称设计选择**：让 encoder / decoder 深度相同，容量大致匹配，结构也更规整。它不是理论推导出来的硬约束，也不是 Transformer 必须满足的规则。

更准确地说：`6` 本身是实验和工程折中的超参数；decoder 层数刚好也取 `6`，是作者在这个 base model 里沿用了和 encoder 相同的配置。后续模型完全可以改成不同层数，比如 encoder-only、decoder-only，或者 encoder/decoder 深度不对称的结构。

也要注意：encoder 和 decoder 的 6 层是**两套独立**的层，不是同一份参数。整个 base model 加起来其实是 12 个 block（encoder 6 + decoder 6）。

#### 句 13：Decoder 多一个 sub-layer：cross-attention
...
这里的 `cross` 可以理解成「跨序列」：Q 和 K/V 不是来自同一个序列，而是 decoder 当前状态去看 encoder 的源句子表示。
...
训练方式本身没有本质区别，仍然是端到端反向传播，`W_Q / W_K / W_V / W_O` 都是可学习参数。真正不同的是**信息来源和 mask 规则**：

- self-attention 学同一个序列内部 token 之间的关系。
- cross-attention 学 decoder 当前位置和 encoder 各位置之间的对应关系。
- decoder self-attention 需要 causal mask，不能看未来目标 token。
- cross-attention 通常不需要 causal mask，因为源句子已经完整给定，decoder 当前 token 可以看 encoder 输出的所有位置。

#### 句 14：Decoder 同样在每个 sub-layer 外面包残差 + LayerNorm
...
#### 句 15：Decoder 的 self-attention 加 causal mask
...
具体实现方式（§3.2.3 里讲）：在 softmax 之前，把「不允许看」的位置的 score 设成 `-inf`，softmax 后那些位置的权重就是 0。先不要看 `0/1` 实现，直接看语义矩阵更清楚（行=query 位置，列=key 位置）：
...
这里 `✓` 表示可以看，`✗` 表示不能看。第 `i` 行表示「位置 `i` 能看见谁」：位置 `i` 只能看见 `0..i`，不能看见 `i+1, i+2, ...`。

容易混淆的是：**mask 里的 `0/1` 不是 attention 权重里的 `0/1`**。mask 发生在 softmax 之前，用来决定哪些 score 要被改成 `-inf`；真正的 attention weight 是 softmax 之后才得到的。被屏蔽的位置在 softmax 后权重才会变成 `0`。

> ⚠️ **代码里的约定不统一，看代码时务必确认**：
> - 屏蔽掩码（block mask）：`1/True` 表示**屏蔽**，`0/False` 表示**可见**，例如 PyTorch `nn.Transformer` 的 `attn_mask`。
> - 有效掩码（keep mask）：`1` 表示**保留/可见**，`0` 表示**屏蔽**，例如 HuggingFace 常见的 `attention_mask`。
> - 自己手写时，看到 `mask` 变量先确认它是「屏蔽掩码」还是「有效掩码」。

另外，实战中除了这个 causal mask，还有 **padding mask**（屏蔽 `<pad>` 位置），两者通常按位 OR 后一起生效。本节论文没展开，但代码里几乎一定会遇到。

#### 句 16：mask + 输出右移一位，共同保证自回归
...
位置:         0     1      2     3
真实目标:     我    喜欢   猫    <EOS>
decoder 输入: <BOS> 我     喜欢  猫     ← 真实目标右移一位、开头补 <BOS>
预测目标:     我    喜欢   猫    <EOS>  ← 与"真实目标"完全相同（不是排版重复）
```
...
这样位置 `i`（从 0 开始）的输入是「真实第 `i-1` 个 token」（位置 0 处是 `<BOS>`），预测目标是「真实第 `i` 个 token」——一句话概括就是「拿前一个词预测下一个词」。
...
先把一个容易卡住的词说清楚：`concat` 就是**拼接**，不是相加。
...
相加:   a + b        = [10, 10, 10]
拼接:   concat(a, b) = [1, 2, 3, 9, 8, 7]
```

在 multi-head attention 里，拼接的意思是：把多个 head 的输出沿特征维首尾接起来，变成一个更长的向量，然后再交给 `W^O` 做线性投影。

### 核心结论

- 不做一次 `d_model` 维 attention，而是把 Q/K/V 投影 `h` 次，每次到更低维度（`d_k`、`d_v`）。
- 每组投影后的 Q/K/V 各自并行做 attention，得到 `h` 个 `d_v` 维输出。
- 把 `h` 个输出拼接（concat）起来，再乘 `W^O` 投回 `d_model`。
...
前面已经说过，`concat` 就是拼接，不是相加。放到 multi-head attention 里，它是沿特征维把多个 head 的输出首尾接起来：
...
`W^O` 不是固定规则，而是一个**可学习的参数矩阵**：它会随机初始化，然后在训练中通过反向传播更新。可以把它理解成 attention 模块的「输出投影层」。
...
它也不是完全「多头才有、单头没有」的概念：很多实现里，单头 attention 也可以有类似的 output projection。只是 multi-head 里 `W^O` 更关键，因为它不仅投回 `d_model`，还负责把多个 head 的结果混合起来。
...
这里隐含了一个训练细节：不同 head 的投影参数需要**随机初始化成不同值**，否则多个 head 会从完全相同的起点出发。

如果两个 head 的参数一开始完全一样：
...
它们看到的输入一样，算出来的 attention 也一样，反向传播得到的梯度也可能一样，最后就容易一直保持一样，退化成「重复的同一个 head」。随机初始化的作用就是**打破这种对称性**，给不同 head 学出不同特征切面的机会。

代码里通常不会手写 `h` 套小矩阵，而是用一个大矩阵一次性算完：
...
这个大矩阵随机初始化后，不同 head 对应的切片天然不同。不过这只表示「有机会不同」，不保证训练后每个 head 都一定学到完全不同的功能；实际模型里有些 head 学到相似模式也很常见。
...
对比 attention 的**主导成本**（`QK^T` 点积 + `softmax @ V` 加权求和）：

```text
单头 full-dim:
  QK^T:        n × n × d_model   = n² × 512
  softmax @ V: n × n × d_model   = n² × 512

多头 (h=8, d_k=d_v=64):
  QK^T:        8 × (n × n × 64)  = n² × 512
  softmax @ V: 8 × (n × n × 64)  = n² × 512
```

主导项总和相同。投影矩阵的参数量也基本守恒：

```text
单头:  W^Q, W^K, W^V, W^O 各 (d_model, d_model)，合计 4 × d_model²
多头:  h 份 (d_model, d_k) 投影 + 一个 (h·d_v, d_model) 的 W^O
       = h × 3 × d_model × d_k + d_model × (h·d_v)
       = 4 × d_model²                         （因为 h·d_k = h·d_v = d_model）
```
