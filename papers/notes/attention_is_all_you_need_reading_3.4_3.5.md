...
这里容易产生一个疑问：两个向量直接相加，会不会把语义信息和位置信息混在一起，导致信息丢失？

答案是：相加不是严格意义上的无损拼接，理论上确实可能混叠；但 Transformer 并不需要还原出“原始 token embedding”和“原始 position encoding”这两份输入，它只需要得到一个对任务有用的混合表示。

因为后续的 Q/K/V 都是线性变换：

```text
(token_embedding + position_encoding) W
= token_embedding W + position_encoding W
```

也就是说，模型可以通过不同投影方向分别利用语义信号和位置信号。位置编码本身有稳定规律，embedding 又会在训练中适应这种注入方式，所以高维相加通常不会成为主要瓶颈。

更准确地说，位置编码不是“让语义变多”，而是给 token 表示补上顺序信息。没有它，self-attention 只能看到一组 token，很难区分“我爱你”和“你爱我”；加上它后，输入变成“我 @ 位置 0”“爱 @ 位置 1”“你 @ 位置 2”。
