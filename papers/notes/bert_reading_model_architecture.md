...
这里其实是在给 **NSP（Next Sentence Prediction，下一句预测）**铺路。

NSP 可以先理解成一个很简单的判断题：给模型两段话 A 和 B，让它判断 **B 是不是 A 原文里真实接着出现的下一段**。正确例子就是两段原本相邻的文本；干扰例子就是把 A 后面随机接上一段不相关的 B。

BERT 为了做这种判断，必须能把两段话一起放进同一次输入里，所以后面会用 `[SEP]` 把 A/B 分开，再给每个词标记“它来自 A 段还是 B 段”。
